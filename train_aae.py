...
def main():
    ...
    discriminator: nn.Module = instantiate_from_config(conf.discriminator)
    ...
    logger.info(f'#params. of discriminator: {sum(p.numel() for p in discriminator.parameters())}')
    logger.info('=' * 50)

    # BUILD OPTIMIZERS
    parameters = list(encoder.parameters()) + list(decoder.parameters())
    optimizer: torch.optim.Optimizer = instantiate_from_config(conf.train.optim, params=parameters)
    optimizer_disc: torch.optim.Optimizer = instantiate_from_config(
        conf.train.optim_disc, params=discriminator.parameters(),
    )
    ...
    encoder, decoder, discriminator, optimizer, optimizer_disc, train_loader = (
        accelerator.prepare(encoder, decoder, discriminator, optimizer, optimizer_disc, train_loader)  # type: ignore
    )
    ...
    def run_step(x):
        ...
        z, _ = encoder(x)   # deterministic encoder
        recx = decoder(z)
    ...
    def run_step_reg(x):
        ...
        z, _ = encoder(x)
        # train discriminator
        fake_score = discriminator(z.detach())
        real_score = discriminator(torch.randn_like(z))
        loss_disc = (F.binary_cross_entropy_with_logits(fake_score, torch.zeros_like(fake_score)) +
                     F.binary_cross_entropy_with_logits(real_score, torch.ones_like(real_score))) / 2
        accelerator.backward(loss_disc)
        optimizer_disc.step()
        # train generator (encoder)
        fake_score = discriminator(z)
        loss_adv = F.binary_cross_entropy_with_logits(fake_score, torch.ones_like(fake_score))
    ...
    while step < conf.train.n_steps:
        ...
        train_status.update(run_step_reg(batch))
